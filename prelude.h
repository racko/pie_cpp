...
constexpr auto plus = define(
    "+", Arrow(Nat, Nat, Nat), lambda([](const auto& lhs, const auto& rhs) { return iter_Nat(lhs, rhs, step_plus); }));
...
constexpr auto times = define(
    "*", Arrow(Nat, Nat, Nat), lambda([](const auto& lhs, const auto& rhs) { return iter_Nat(lhs, zero, plus(rhs)); }));
...
constexpr auto incr = define("incr", Arrow(Nat, Nat), lambda([](const auto& n) { return iter_Nat(n, one, plus(one)); }));

constexpr auto base_incr_equals_add1 = define("base-incr=add1", Eq(Nat, incr(zero), add1(zero)), same(add1(zero)));

constexpr auto mot_incr_equals_add1 = define("mot-incr=add1", Arrow(Nat, U), lambda([](const auto& k){ return Eq(Nat, incr(k), add1(k)); }));

constexpr auto step_incr_equals_add1 = define("step-incr=add1",
                                              Pi(Nat,
                                                 [](const auto& smaller) {
                                                     return Arrow(Eq(Nat, incr(smaller), add1(smaller)),
                                                                  Eq(Nat, add1(incr(smaller)), add1(add1(smaller))));
                                                 }),
